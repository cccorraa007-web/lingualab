...
const EXAMINER_SYSTEM = `你是一名西班牙语口语考官，正在引导一位中文母语学习者练习西班牙语口语。
...
const POLISH_SYSTEM = `你是西班牙语教学专家，擅长帮助中文母语学习者把口语表达改得地道、流畅、丰富。
...
- error_type：核心错误的类型，取值：介词搭配 / 比较结构 / 单词误用 / 动词搭配 / 冠词 / 时态 / 语序 / 性数错误 / 中式西语 / 其他。其中「性数错误」指形容词、冠词、代词与名词的性（阴阳性）或数（单复数）不一致；「中式西语」指直译中文造成的别扭表达
- wrong：错误的核心部分，要完整到能独立说明这个错误。搭配类错误（介词/动词/冠词搭配）要包含核心词与介词/冠词（例如「diferencia en」而不是只写「en」）；单词误用则写错词本身（如「afecto」）；若只是扩写优化而无明确错误则为空字符串
- correct：对应的正确部分（例如「diferencia entre」、「impacto」）；同上可为空
  ...
  const result = await chatJSON<{ prompt?: string }>([
    {
      role: "system",
      content:
        "你是西班牙语老师。生成一个简短自然的中文句子作为口译练习的提示。",
    },
    {
      role: "user",
      content: `请生成一个中文句子，要求：把这个中文句子翻译成西班牙语时，应该会用到这个西语表达「${mistake.correct}」。句子要自然、口语化、简短。只输出 JSON {"prompt": "中文句子"}`,
    },
  ]);
  ...
  const result = await chatJSON<{ correct?: boolean; feedback?: string }>([
    {
      role: "system",
      content:
        "你是西班牙语老师，正在给学习者做口译练习评分。只输出 JSON。",
    },
    {
      role: "user",
...
- 忽略标点符号和大小写差异（如 "así, que" 和 "así que" 视为相同），只关注核心表达是否正确
...
export async function generateExamQuestion(
  ...
  const typeDesc: Record<string, string> = {
    ...
    t3: "Tarea 3（情景对话·留言式）：生成一个具体的情景任务，让考生以「留言」的形式完成沟通。常见类型：提要求、拒绝邀请、获取信息、约定（约时间/见面）、取消约定。题目要包含明确的情景设定（我是谁、面对谁、什么场合）和沟通目的。例如：你是一个来自农村的学生，给当地政府留言，请求增加村子的网络基础设施建设，方便孩子们上网课。",
  ...
  const result = await chatJSON<{ question?: string }>([
    ...
    {
      role: "user",
      content: `请生成一道 ${typeDesc[type] ?? typeDesc.t1}\n\n可参考的语料内容（口语练习题目）：\n${context}\n\n要求：题目用西班牙语，情景化、具体、贴近真实考试。只输出 JSON {"question": "题目内容"}`,
    },
  ]);
