...
- 笔头作业拆为列表、作业详情、单个学生批改、班级分析四级路由；列表仅保留发布与状态卡片，学生提交和教师批改分别进入独立页面。
- 评分新增 A+ / A / B+ / B / C+ / C / D 等级字段，保留旧 `score` 字段兼容历史数据；学生档案改为等级分布。
- 新增作业发布、学生提交、教师批改三条通知链路，通知携带独立 `assignment_id` 并深链到作业详情。
- 新增独立的笔头作业班级分析 AI 与接口，只读 `assignment_submissions`、只写 `classroom_assignments`，不接触必读文章数据表。
- 写作润色新增独立历史记录表、保存逻辑、历史列表和详情页；保存失败不影响当次 AI 润色结果。
- 已批改作业可将题目、正文/OCR 和教师反馈导入写作润色；学生档案新增安全聚合排行接口，只公开前三名必要指标。
- 新增迁移：`migrate_assignments_grade.sql`、`migrate_assignments_analysis.sql`、`migrate_assignments_notifications.sql`、`migrate_writing_sessions.sql`。

### 8.2 必读文章多级结构

- 必读文章拆为多级页面：二级文章页（原文 + 题目列表 + 备课笔记）、三级题目页（教师看已答/未答学生、学生作答）、四级作答页（教师批注留言）。
- 学生端文章页新增学习引导 + 一键加入语料库；教师端题目页新增删除题目。
- 新增题目详情接口 `GET /api/classrooms/[id]/readings/[readingId]/questions/[questionId]` 与学生作答接口 `/answer`、教师批注接口 `/answers/[answerId]/feedback`，作答/批改会互相通知对方。

### 8.3 班级作答分析改为文章级

- 将「班级作答分析」从题目详情页迁移到教师阅读二级页：文章页新增「班级作答分析」入口，点击进入三级分析详情页。
...
### 8.7 教学-自学联动架构总览（含我的档案）

- 整体定位：教学模式负责「课内」，自学模式负责「课外」，通过联动打通：
  - 学生·必读文章 ↔ 语料库：教师批改后学生可「加入语料库」复习（已上线）。
  - 学生·笔头作业 ↔ 写作润色：教师批改后学生可「加入写作润色」（题目→作文题、作答/OCR→正文、批注反馈→评分建议），交由队友实现。
- 学生模式：
  - 「课后作业」= 必读文章（课前预习）+ 笔头作业（课后作业）。
  - 「班级成员」改为「我的档案」（`/teaching/[id]/profile`）：二级含 班级成员 + 我的作业提交情况 + 作业评分统计仪表盘 + 班级综合排行前三（排行需队友提供聚合数据）。
  - 「课外练习」直接链接到自学模式（`/practice`）。
- 教师模式：
  - 「布置作业」= 作业收发 + 平时成绩追踪：必读文章看学生答题/提问调整上课重点；笔头作业按完成评级与按时提交记录平时分。
  - 「备课资料库」（`/teaching/[id]/speaking`，已上线初版）：从 语料库 / 发布的必读文章 / 发布的笔头作业 三处导入素材，勾选后按需求（格式/题型/题量/单词讲解）生成针对性课件；新增 `POST /api/classrooms/[id]/library`（仅教师）。

### 8.8 必读文章备课流程重构 + 备课资料库收藏

- 取消必读文章里的「直接备课模式」（`?prep=1`、辅助备课、生成辅助课件按钮全部移除）。
- 教师端必读文章页新增「添加到备课资料库」提示卡片：点击把该文章收藏进 `lesson_library`，收藏后显示「去备课」按钮跳转备课资料库。
- 新增 `db/migrate_lesson_library.sql`（`lesson_library` 表）+ `POST/GET /api/classrooms/[id]/library/items` 与 `GET .../items/[itemId]`。
...
### 8.12 写作交互与教师端档案/备课完善

- 写作润色成功后将题目、正文和评分标准转为结果快照并清空输入；结果页新增原文记录，按 `polish[].original` 多片段匹配合并后标红下划线。加入错题本成功后清空结果并回到新作文初始状态。
- 班级成员页新增教师专属「学生档案」入口，展示全班平时作业、预习、等级分布及课外阅读/口语/写作/错题统计，并汇总已生成的预习和课后作业班级分析。
